...
export function createSelectors<TState extends Record<string, any>>(store: StoreApi<TState>) {
  const selectors = {} as {
  ...
  stateKeys.forEach((key) => {
    const selectorName = `use${capitalize(key as string)}` as `use${Capitalize<string & keyof TState>}`;

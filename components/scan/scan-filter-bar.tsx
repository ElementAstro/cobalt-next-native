...
const ScanFilterBar: React.FC<ScanFilterBarProps> = ({
  ...
  const updateFilter = useCallback(
    (key: keyof FilterOptions, value: any) => {
      ...
      const filterName = 
        ...
        key === "portRange" ? "端口范围" :
        key === "sortBy" ? "排序方式" : "排序方向";
        
      const filterValueText = 
        value === null ? "已重置" :
        key === "status" && value === "open" ? "开放" :
        key === "status" && value === "closed" ? "关闭" :
        key === "status" && value === "filtered" ? "过滤" :
        key === "status" && value === "error" ? "错误" :
        key === "sortBy" && value === "port" ? "端口号" :
        key === "sortBy" && value === "status" ? "状态" :
        key === "sortBy" && value === "service" ? "服务名称" :
        key === "sortBy" && value === "latency" ? "延迟" :
        key === "sortDirection" && value === "asc" ? "升序" :
        key === "sortDirection" && value === "desc" ? "降序" :
        String(value);
    ...
    <Tooltip>
      <TooltipTrigger>
        <Button
          ...
          `}
          accessibilityLabel={`${title}过滤器: ${options.find(o => o.value === currentValue)?.label || "未选择"}`}
          accessibilityHint="点击更改过滤选项"
        >
          <IconComponent className={`h-4 w-4 mr-1 ${currentValue !== null ? "text-primary" : "text-muted-foreground"}`} />
          <Label className={`text-xs ${currentValue !== null ? "text-primary" : ""}`}>
  ...
  const renderFilterTags = () => {
    ...
    return (
      ...
      >
        ...
        {filters.serviceType !== null && (
          <FilterTag 
            label={serviceOptions.find(o => o.value === filters.serviceType)?.label || ""}
        ...
        {filters.sortBy !== "port" && (
          <FilterTag 
            label={`按${sortByOptions.find(o => o.value === filters.sortBy)?.label || ""}排序`}
  ...
  // 过滤器标签组件
  const FilterTag = ({ label, onPress }: { label: string; onPress: () => void }) => (
        ...
        <Text className="text-xs text-muted-foreground">
          {isLoading ? (
            <View className="flex-row items-center">
              <Loader2 size={10} className="text-muted-foreground animate-spin mr-1" />
    ...
    >
      ...
      <View className="px-4 pt-2 pb-1 flex-row items-center space-x-3">
        ...
        <AnimatedPressable
          ...
          accessibilityLabel={`排序方向：${filters.sortDirection === "asc" ? "升序" : "降序"}`}
          accessibilityRole="button"
        ...
        {showFilters && (
          <AnimatedPressable
            ...
            accessibilityLabel={`过滤器${activeFilterCount > 0 ? `：已应用${activeFilterCount}个` : ""}`}
            accessibilityRole="button"
      ...
      {/* 展开的过滤器面板 */}
      {showFilters && (
        <Animated.View style={filterContainerStyle} className="overflow-hidden px-4">

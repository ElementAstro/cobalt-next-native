...
export default function UpdateDialog() {
  ...
  const updateInfo = useMemo(() => ({
    ...
    isNewer: available?.runtimeVersion && currentlyRunning.runtimeVersion
      ...
      : false,
    versionName: available?.manifest?.version || available?.manifest?.metadata?.expoClient?.version || "未知",
  ...
  const updateDetails = useMemo<UpdateDetails | null>(() => {
    ...
    return {
      ...
      description: "此更新包含了多项功能改进和错误修复，提升了应用的稳定性和性能。",
    ...
    try {
      ...
      const { manifest } = await (Updates as any).fetchUpdateAsync({
        onProgress: ({ totalBytes, downloaded }: { totalBytes: number; downloaded: number }) => {
          ...
          // 在关键点提供触觉反馈
          if ([25, 50, 75, 100].includes(Math.round(normalizedProgress)) && 
      ...
      checkmarkScale.value = withSequence(
        withDelay(300, withTiming(1, { duration: 300, easing: Easing.out(Easing.back) }))
      );
  ...
  const toggleChangelog = useCallback(() => {
    ...
    changelogHeight.value = withTiming(showChangelog ? 0 : 1, {
      duration: 300,
      easing: showChangelog ? Easing.in(Easing.cubic) : Easing.out(Easing.cubic)
    });
  ...
  // 渲染变更日志项
  const renderChangelogItem = useCallback((item: ChangelogItem, index: number) => {
    const getIconAndColor = () => {
      switch (item.type) {
        case 'feature':
          return { icon: <Sparkles size={14} className="text-blue-500" />, color: "text-blue-500" };
        case 'fix':
          return { icon: <Check size={14} className="text-emerald-500" />, color: "text-emerald-500" };
        case 'improvement':
          return { icon: <ArrowRight size={14} className="text-amber-500" />, color: "text-amber-500" };
        case 'security':
          return { icon: <Shield size={14} className="text-purple-500" />, color: "text-purple-500" };
        default:
          return { icon: <Info size={14} className="text-slate-500" />, color: "text-slate-500" };
      }
    };
    ...
    return (
      ...
      >
        {icon}
        <Text className={`text-sm ${color} flex-1`}>
          {item.description}
        </Text>
      </Animated.View>
    );
      ...
      >
        <View className="space-y-4 pt-2">
          <View className="space-y-2">
            <Text className="text-base font-medium text-foreground">{updateDetails.title}</Text>
            <Text className="text-sm text-muted-foreground">{updateDetails.description}</Text>
          </View>
  ...
  }, [updateDetails, expandStyle, changelogStyle, showChangelog, renderChangelogItem]);
        ...
        <CardContent className="p-4 space-y-4">
          ...
          {updateInfo.hasUpdate && (
            <AnimatedPressable
              ...
              accessibilityHint={activeSection === 'overview' ? "查看更多信息" : "返回概览"}
            >
              <View className="flex-row items-center space-x-2">
              ...
              <ChevronRight 
                ...
                style={{ 
                  transform: [{ rotate: activeSection === 'details' ? '90deg' : '0deg' }]
                }} 
              />
            </AnimatedPressable>
          )}

...
export const FileList: React.FC<FileListProps> = ({
  ...
  // 视图模式变更同步到动画值
  useEffect(() => {
    activeViewMode.value = withTiming(viewMode === "grid" ? 0 : 1, {
      duration: 300,
  ...
  const scrollHandler = useAnimatedScrollHandler({
    onScroll: (event) => {
      ...
      if (scrollY.value > prevScrollY + 5 && switcherVisible.value === 1) {
        // 向下滚动隐藏
        switcherVisible.value = withTiming(0, {
          duration: 300,
          easing: Easing.out(Easing.quad),
        });
      } else if (
        scrollY.value < prevScrollY - 5 &&
        switcherVisible.value === 0
      ) {
        // 向上滚动显示
        switcherVisible.value = withTiming(1, {
          duration: 300,
          easing: Easing.out(Easing.quad),
        });
      }
      ...
      // 控制回到顶部按钮显示
      if (
        event.contentOffset.y > SCREEN_HEIGHT * 0.3 &&
        fabOpacity.value === 0
      ) {
  ...
  const gridIconStyle = useAnimatedStyle(() => {
    return {
      ...
      transform: [
        {
          scale: withTiming(1 - activeViewMode.value * 0.3, { duration: 200 }),
        },
      ],
    };
  });
  ...
  const listIconStyle = useAnimatedStyle(() => {
    return {
      ...
      transform: [
        {
          scale: withTiming(0.7 + activeViewMode.value * 0.3, {
            duration: 200,
          }),
        },
      ],
  ...
  const renderSkeletonItem = useCallback(
    () => (
      <Animated.View entering={FadeIn.duration(300)} className="p-2">
  ...
  const renderLoadingState = useCallback(() => {
    const skeletonCount = Math.max(
      6,
      Math.floor((SCREEN_HEIGHT / 200) * columnCount)
    );
      ...
      <View className="flex-row flex-wrap">
        {Array.from({ length: skeletonCount }).map((_, index) => (
          <View
            key={`skeleton-${index}`}
            style={{
              width: viewMode === "grid" ? `${100 / columnCount}%` : "100%",
            }}
            className="p-2"
        ...
        >
          ...
          >
            <Button
              ...
              accessibilityHint={`当前是${
                viewMode === "grid" ? "网格" : "列表"
              }模式，点击切换到${viewMode === "grid" ? "列表" : "网格"}模式`}
            >
              <Animated.View
                style={gridIconStyle}
